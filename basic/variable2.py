# variable 변수 - 타입이 없음(할당하면 타입이 생김)
# str, int, float, bool
...
# find('찾을문자',시작위치) : 찾는 문자열이 없으면 -1 표시
print("특정 문자가 시작되는 첫번째 위치 반환",str1.find("i"))
print("특정 문자가 시작되는 첫번째 위치 반환",str1.find("k"))
print("특정 문자가 시작되는 첫번째 위치 반환",str1.find("i",4)) # 4번째 이후부터 찾아라

# index() : 찾는문자열이 없다면 에러 발생
...
# 규칙3 : 남은 글자 중 처음 세자리 + 글자 갯수 + 글자 내 e 문자 개수 + ! 로 구성 => nav + 5 + 1 + !
# 결과 : nav51!
print()
print("실습 : 암호생성")
url = "http://naver.com"
...
# 숫자를 str로 감싸줘야함
password = url[:3]+str(len(url))+str(e_cnt)+"!" # ypeError: can only concatenate str (not "int") to str
print(password)
